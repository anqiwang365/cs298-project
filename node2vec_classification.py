...
def extract_feature():
    ...
    text2content = {}  # key:textId, value:textContent
    text2user = {}  # key:textId value:userId
    text2label = {}  # key:textId value:label
    all_data = []
    all_label = []
    with open('tweet_dataset_edited.csv', encoding="utf8", errors='ignore', mode='r') as csvfile:
        data = csv.reader(csvfile, delimiter=',')
        for row in data:
            text2content[row[0]] = preprocess_data(row[1])
            text2label[row[0]] = row[4]  # 0:cyber; 1:normal
            all_data.append(preprocess_data(row[1]))
            all_label.append(row[4])
            text2user[row[0]] = row[2]
    train_data, data = train_test_split(all_data, test_size=0.3, random_state=0)
    vec = TfidfVectorizer(binary=True, use_idf=True)
    text_features = vec.fit_transform(train_data)

    posData = []
    negData = []
    poslabel = []
    neglabel = []
    for textId in text2content.keys():
        list = vec.transform([text2content[textId]])
        list = list.toarray()[0].tolist()
        userId = text2user[textId]
        embed = embedding.get(userId, numpy.zeros(128)).tolist()
        feature = list + embed
        label = text2label[textId]
        if label == '1':
